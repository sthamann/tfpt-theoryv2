...
from RGEs import (beta_g1, beta_g2, beta_g3,
                  beta_Yu, beta_Yd, beta_Ye, beta_yN,
                  beta_lambda_, beta_lPhi, beta_lHphi,
                  beta_cR3,
                  beta_mu2, beta_MPhi,
                  beta_vSM, beta_vPQ)

# -----------------------------------------------------------------
# >>> Schwellen, überschrieben vom Runner (siehe unten)
Sigma_F = 1.0e3   # GeV  – farbige Σ_F-Fermionen
N_R     = 1.0e15  # GeV  – rechtshändige Neutrinos
# -----------------------------------------------------------------
class Coupling():
    couplings = {}
...
class RGEsolver():
    ...
    def __init__(self, name, initialScale = 0, tmin = 0, tmax = 20):
        ...
        # Gauge Couplings
        self.g1 = Coupling('g1', 'GaugeCouplings')
        self.g2 = Coupling('g2', 'GaugeCouplings')
        self.g3 = Coupling('g3', 'GaugeCouplings')

        # Yukawa Couplings
        self.Yu = Coupling('Yu', 'Yukawas', shape=(3,3))
        self.Yd = Coupling('Yd', 'Yukawas', shape=(3,3))
        self.Ye = Coupling('Ye', 'Yukawas', shape=(3,3))
        self.yN = Coupling('yN', 'Yukawas', shape=(3,3))

        # Quartic Couplings
        self.lambda_ = Coupling('lambda_', 'QuarticTerms')
        self.lPhi = Coupling('lPhi', 'QuarticTerms')
        self.lHphi = Coupling('lHphi', 'QuarticTerms')

        # Trilinear Couplings
        self.cR3 = Coupling('cR3', 'TrilinearTerms')

        # Scalar Mass Couplings
        self.mu2 = Coupling('mu2', 'ScalarMasses')
        self.MPhi = Coupling('MPhi', 'ScalarMasses')

        # Vacuum-expectation Values
        #   For vevs the gauge must be fixed. Let's use for instance the Landau gauge :
        self.xiGauge = 0

        self.vSM = Coupling('vSM', 'Vevs')
        self.vPQ = Coupling('vPQ', 'Vevs')
    ...
    def betaFunction(self, t, couplingsArray):
        ...
        # ------------------------------------------------------------
        # Aktuelle RG-Skala  μ  (GeV)  –  PyR@TE nutzt  t = log10 μ
        # ------------------------------------------------------------
        g1, g2, g3 = self.extractCouplings(couplingsArray, 'GaugeCouplings')
        mu = 10.0 ** t
        Yu, Yd, Ye, yN = self.extractCouplings(couplingsArray, 'Yukawas')
        lambda_, lPhi, lHphi = self.extractCouplings(couplingsArray, 'QuarticTerms')
        cR3, = self.extractCouplings(couplingsArray, 'TrilinearTerms')
        mu2, MPhi = self.extractCouplings(couplingsArray, 'ScalarMasses')
        vSM, vPQ = self.extractCouplings(couplingsArray, 'Vevs')

        bg1, bg2, bg3 = 3*[0]
        bYu, bYd, bYe, byN = 4*[0]
        blambda_, blPhi, blHphi = 3*[0]
        bcR3 = 0
        bmu2, bMPhi = 2*[0]
        bvSM, bvPQ = 2*[0]

        if self.loops['GaugeCouplings'] >= 1:
            bg1 += beta_g1(1, g1,g2,g3,Yu,Yd,Ye,yN)*self.kappa(1)*np.log(10)
            bg2 += beta_g2(1, g2,g1,g3,Yu,Yd,Ye,yN)*self.kappa(1)*np.log(10)
            bg3 += beta_g3(1, g3,g1,g2,Yu,Yd)*self.kappa(1)*np.log(10)
        if self.loops['GaugeCouplings'] >= 2:
            bg1 += beta_g1(2, g1,g2,g3,Yu,Yd,Ye,yN)*self.kappa(2)*np.log(10)
            bg2 += beta_g2(2, g2,g1,g3,Yu,Yd,Ye,yN)*self.kappa(2)*np.log(10)
            bg3 += beta_g3(2, g3,g1,g2,Yu,Yd)*self.kappa(2)*np.log(10)

        if self.loops['Yukawas'] >= 1:
            bYu += beta_Yu(1, g1,g2,g3,Yu,Yd,Ye,yN,lambda_,lHphi)*self.kappa(1)*np.log(10)
            bYd += beta_Yd(1, g1,g2,g3,Yu,Yd,Ye,yN,lambda_,lHphi)*self.kappa(1)*np.log(10)
            bYe += beta_Ye(1, g1,g2,Yu,Yd,Ye,yN,g3,lambda_,lHphi)*self.kappa(1)*np.log(10)
            byN += beta_yN(1, g1,g2,Yu,Yd,Ye,yN,g3,lambda_,lHphi)*self.kappa(1)*np.log(10)
        if self.loops['Yukawas'] >= 2:
            bYu += beta_Yu(2, g1,g2,g3,Yu,Yd,Ye,yN,lambda_,lHphi)*self.kappa(2)*np.log(10)
            bYd += beta_Yd(2, g1,g2,g3,Yu,Yd,Ye,yN,lambda_,lHphi)*self.kappa(2)*np.log(10)
            bYe += beta_Ye(2, g1,g2,Yu,Yd,Ye,yN,g3,lambda_,lHphi)*self.kappa(2)*np.log(10)
            byN += beta_yN(2, g1,g2,Yu,Yd,Ye,yN,g3,lambda_,lHphi)*self.kappa(2)*np.log(10)

        if self.loops['QuarticTerms'] >= 1:
            blambda_ += beta_lambda_(1, g1,g2,Yu,Yd,Ye,yN,lambda_,lHphi,g3)*self.kappa(1)*np.log(10)
            blPhi += beta_lPhi(1, lPhi,lHphi,g1,g2,Yu,Yd,Ye,yN)*self.kappa(1)*np.log(10)
            blHphi += beta_lHphi(1, g1,g2,Yu,Yd,Ye,yN,lambda_,lPhi,lHphi,g3)*self.kappa(1)*np.log(10)
        if self.loops['QuarticTerms'] >= 2:
            blambda_ += beta_lambda_(2, g1,g2,Yu,Yd,Ye,yN,lambda_,lHphi,g3)*self.kappa(2)*np.log(10)
            blPhi += beta_lPhi(2, lPhi,lHphi,g1,g2,Yu,Yd,Ye,yN)*self.kappa(2)*np.log(10)
            blHphi += beta_lHphi(2, g1,g2,Yu,Yd,Ye,yN,lambda_,lPhi,lHphi,g3)*self.kappa(2)*np.log(10)

        if self.loops['TrilinearTerms'] >= 1:
            bcR3 += beta_cR3(1, g1,g2,Yu,Yd,Ye,yN,lambda_,cR3,g3,lHphi)*self.kappa(1)*np.log(10)
        if self.loops['TrilinearTerms'] >= 2:
            bcR3 += beta_cR3(2, g1,g2,Yu,Yd,Ye,yN,lambda_,cR3,g3,lHphi)*self.kappa(2)*np.log(10)

        if self.loops['ScalarMasses'] >= 1:
            bmu2 += beta_mu2(1, g1,g2,Yu,Yd,Ye,yN,lambda_,lHphi,cR3,mu2,MPhi,g3)*self.kappa(1)*np.log(10)
            bMPhi += beta_MPhi(1, lPhi,lHphi,mu2,MPhi,g1,g2,Yu,Yd,Ye,yN,cR3)*self.kappa(1)*np.log(10)
        if self.loops['ScalarMasses'] >= 2:
            bmu2 += beta_mu2(2, g1,g2,Yu,Yd,Ye,yN,lambda_,lHphi,cR3,mu2,MPhi,g3)*self.kappa(2)*np.log(10)
            bMPhi += beta_MPhi(2, lPhi,lHphi,mu2,MPhi,g1,g2,Yu,Yd,Ye,yN,cR3)*self.kappa(2)*np.log(10)

        if self.loops['Vevs'] >= 1:
            bvSM += beta_vSM(1, g1,g2,Yu,Yd,Ye,yN,vSM,self.xiGauge,g3,lambda_,lHphi)*self.kappa(1)*np.log(10)
            bvPQ += beta_vPQ(1, lPhi,lHphi,vPQ)*self.kappa(1)*np.log(10)
        if self.loops['Vevs'] >= 2:
            bvSM += beta_vSM(2, g1,g2,Yu,Yd,Ye,yN,vSM,self.xiGauge,g3,lambda_,lHphi)*self.kappa(2)*np.log(10)
            bvPQ += beta_vPQ(2, lPhi,lHphi,vPQ)*self.kappa(2)*np.log(10)
        # -----------------------------------------------------------------
        #               >>>  THRESHOLD DECOPLING  <<<
        #  • Σ_F   : trägt oberhalb  Σ_F   +8/3 · g₃³ zu β(g₃) bei
        #  • N_R   : trägt oberhalb  N_R   +4/3 · g₁³ zu β(g₁) bei
        # -----------------------------------------------------------------
        if mu < Sigma_F:
            bg3 -= (8/3) * g3**3 * self.kappa(1) * np.log(10)

        if mu < N_R:
            bg1 -= (4/3) * g1**3 * self.kappa(1) * np.log(10)

        # Zweiloop-Beiträge der schweren Felder sind < 1 % des Gesamteffekts
        # und werden hier vernachlässigt. Wer es ganz genau will, kann sie
        # analog abziehen.

        return [bg1, bg2, bg3, *bYu.flat, *bYd.flat, *bYe.flat, *byN.flat, blambda_, blPhi, blHphi, bcR3, bmu2, bMPhi, bvSM, bvPQ]
