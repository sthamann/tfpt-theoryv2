...
class RGEsolver():
    ...
    def betaFunction(self, t, couplingsArray):
        ...
        if self.loops['GaugeCouplings'] >= 1:
            bg1 += beta_g1(1, g1)*self.kappa(1)*np.log(10)
            bg2 += beta_g2(1, g2)*self.kappa(1)*np.log(10)
            bg3 += beta_g3(1, g3)*self.kappa(1)*np.log(10)

        if self.loops['Yukawas'] >= 1:
            bYu += beta_Yu(1, g1,g2,g3,Yu,Yd,Ye)*self.kappa(1)*np.log(10)
            bYd += beta_Yd(1, g1,g2,g3,Yu,Yd,Ye)*self.kappa(1)*np.log(10)
            bYe += beta_Ye(1, g1,g2,Yu,Yd,Ye)*self.kappa(1)*np.log(10)

        if self.loops['QuarticTerms'] >= 1:
            blambda_ += beta_lambda_(1, g1,g2,Yu,Yd,Ye,lambda_)*self.kappa(1)*np.log(10)

        if self.loops['ScalarMasses'] >= 1:
            bmu2 += beta_mu2(1, g1,g2,Yu,Yd,Ye,lambda_,mu2)*self.kappa(1)*np.log(10)

        if self.loops['Vevs'] >= 1:
            bvSM += beta_vSM(1, g1,g2,Yu,Yd,Ye,vSM,self.xiGauge)*self.kappa(1)*np.log(10)
