...
#  Author : TFPT (v2.5) – Standard Model 2-loop baseline for RG fingerprints#
#  Date   : 2026-01-24                                  #
#########################################################
...
class RGEsolver():
    ...
    def betaFunction(self, t, couplingsArray):
        ...
        Yu, Yd, Ye = self.extractCouplings(couplingsArray, 'Yukawas')
        lambda_, = self.extractCouplings(couplingsArray, 'QuarticTerms')
        mu2, = self.extractCouplings(couplingsArray, 'ScalarMasses')
        vSM, = self.extractCouplings(couplingsArray, 'Vevs')
        ...
        if self.loops['GaugeCouplings'] >= 1:
            bg1 += beta_g1(1, g1,g2,g3,Yu,Yd,Ye)*self.kappa(1)*np.log(10)
            bg2 += beta_g2(1, g2,g1,g3,Yu,Yd,Ye)*self.kappa(1)*np.log(10)
            bg3 += beta_g3(1, g3,g1,g2,Yu,Yd)*self.kappa(1)*np.log(10)
        if self.loops['GaugeCouplings'] >= 2:
            bg1 += beta_g1(2, g1,g2,g3,Yu,Yd,Ye)*self.kappa(2)*np.log(10)
            bg2 += beta_g2(2, g2,g1,g3,Yu,Yd,Ye)*self.kappa(2)*np.log(10)
            bg3 += beta_g3(2, g3,g1,g2,Yu,Yd)*self.kappa(2)*np.log(10)

        if self.loops['Yukawas'] >= 1:
            bYu += beta_Yu(1, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(1)*np.log(10)
            bYd += beta_Yd(1, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(1)*np.log(10)
            bYe += beta_Ye(1, g1,g2,Yu,Yd,Ye,g3,lambda_)*self.kappa(1)*np.log(10)
        if self.loops['Yukawas'] >= 2:
            bYu += beta_Yu(2, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(2)*np.log(10)
            bYd += beta_Yd(2, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(2)*np.log(10)
            bYe += beta_Ye(2, g1,g2,Yu,Yd,Ye,g3,lambda_)*self.kappa(2)*np.log(10)

        if self.loops['QuarticTerms'] >= 1:
            blambda_ += beta_lambda_(1, g1,g2,Yu,Yd,Ye,lambda_,g3)*self.kappa(1)*np.log(10)
        if self.loops['QuarticTerms'] >= 2:
            blambda_ += beta_lambda_(2, g1,g2,Yu,Yd,Ye,lambda_,g3)*self.kappa(2)*np.log(10)

        if self.loops['ScalarMasses'] >= 1:
            bmu2 += beta_mu2(1, g1,g2,Yu,Yd,Ye,lambda_,mu2,g3)*self.kappa(1)*np.log(10)
        if self.loops['ScalarMasses'] >= 2:
            bmu2 += beta_mu2(2, g1,g2,Yu,Yd,Ye,lambda_,mu2,g3)*self.kappa(2)*np.log(10)

        if self.loops['Vevs'] >= 1:
            bvSM += beta_vSM(1, g1,g2,Yu,Yd,Ye,vSM,self.xiGauge,g3,lambda_)*self.kappa(1)*np.log(10)
        if self.loops['Vevs'] >= 2:
            bvSM += beta_vSM(2, g1,g2,Yu,Yd,Ye,vSM,self.xiGauge,g3,lambda_)*self.kappa(2)*np.log(10)

        return [bg1, bg2, bg3, *bYu.flat, *bYd.flat, *bYe.flat, blambda_, bmu2, bvSM]
