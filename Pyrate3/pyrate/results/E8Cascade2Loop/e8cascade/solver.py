...
class E8CascadeSolver:
    ...
    def __init__(self, model_package, model_module, results_dir, *,
        ...
        # Set gravity portal coefficient (if cR3 exists)
        if hasattr(self.model, 'cR3'):
            ...
            print("Note: No trilinear coupling cR3 in this model")
        
        self.enable_grav = enable_gravity_portal
        self.cR_factor = 1.0  # Can be modified for parameter studies
    
    def _set_initial_conditions(self):
        """Set realistic initial values at M_Z - EXACTLY like working E8Cascade2LoopGravityV2"""
        
        # Gauge couplings - USE DIRECT SM VALUES (like the working model!)
        # PyR@TE will handle the GUT normalization internally
        self.model.g1.initialValue = 0.357  # SM normalization (like working model)
        self.model.g2.initialValue = 0.652
        self.model.g3.initialValue = 1.221
        
        print(f"✓ Gauge couplings set (SM normalization, PyR@TE handles GUT):")
        print(f"  g1_SM = {self.model.g1.initialValue} (will become g1_GUT internally)")
        print(f"  g2 = {self.model.g2.initialValue}")
        print(f"  g3 = {self.model.g3.initialValue}")
        
        # Yukawa couplings (only 3rd generation significant)
        ...
        # Right-handed neutrinos - CORRECTED with non-zero seesaw couplings!
        # These drive the RG evolution towards unification
        self.model.yN1.initialValue = [[0.70], [0.0], [0.0]]  # 1st generation
        self.model.yN2.initialValue = [[0.0], [0.70], [0.0]]  # 2nd generation  
        self.model.yN3.initialValue = [[0.0], [0.0], [0.70]]  # 3rd generation
        
        print(f"✓ Seesaw neutrino couplings activated: yN1 = yN2 = yN3 = 0.70")
        
        # Scalar couplings - CORRECTED with YAML values for cascade physics!
        self.model.lambda_.initialValue = 0.130  # Higgs self-coupling (from YAML)
        self.model.lPhi.initialValue = 0.10      # PQ scalar self-coupling (from YAML)
        self.model.lHphi.initialValue = 0.01     # Higgs-PQ mixing (from YAML)
        
        print(f"✓ PQ scalar couplings: λ_Φ = {self.model.lPhi.initialValue}, λ_Hφ = {self.model.lHphi.initialValue}")
